...
- Unless you enable resource sync in Azure IoT Operations using [`az iot ops enable-rsync`](/cli/azure/iot/ops/rsync#az-iot-ops-enable-rsync), changes made to the resources using Kubernetes deployment manifests are not synced to Azure. To learn more about resource sync, see [Resource sync](/azure/azure-arc/data/resource-sync).
