...
| Mqtt.SuccessfulPublishLatencyInMilliseconds	| Mqtt.SuccessfulPublishLatencyInMilliseconds |	Milliseconds | Total | The observed latency in milliseconds between the time namespace receives an MQTT PUBLISH packet at QoS 1 and the time it sends the corresponding PUBACK acknowledgment to the client. QoS 0 messages aren’t included in this metric. | Protocol, QoS, Result, Error |
