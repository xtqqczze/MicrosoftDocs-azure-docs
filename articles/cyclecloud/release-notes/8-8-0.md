...
- Azure CycleCloud node arrays now support attaching precreated Virtual Machine Scale Sets (aka bring-your-own Virtual Machine Scale Sets) by setting the new `PredefinedScaleSetId` node attribute. 
...
- Using the Azure CLI `az vm run-command` on a Azure CycleCloud node caused Azure CycleCloud to flag the node as failed with the message: "An unspecified error occurred."
