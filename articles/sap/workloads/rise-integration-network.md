...
For customers requiring stronger network-based security than NSGs, RISE offers an optional service called "Firewall as a Service (FWaaS)". The firewall is a managed service provided by SAP RISE/ECS and provides macro-segmentation of SAP workloads into different tiers. Systems within the same tier can bypass the firewall and communicate directly, while inter-tier communication has to pass through the firewall and follow traffic control rules. You can modify the necessary rule set and obtain firewall logs and audit trails through RISE LogServ service.

To enable the FWaaS service, [subnet peering](../../virtual-network/how-to-configure-subnet-peering.md) will be established between one subnet from the RISE VNet and one subnet on the customer side. (Unlike full VNet peering, subnet peering will only enable communication between the selected subnets involved in the peering.) When you subscribe to RISE's FWaaS offering, subnet peering will be established between a RISE subnet hosting a firewall and a customer subnet hosting an NVA or other routing appliance. Each NVA is configured as the primary route target for all RISE communication. As a result, all traffic between SAP RISE workloads and your network will thus pass through both appliances.
...
:::image type="complex" source="./media/sap-rise-integration/sap-rise-fwaas.png" alt-text="Diagram of SAP RISE/ECS firewall as a service design.":::
   This diagram shows a typical SAP customer's hub and the RISE virtual network. Firewalls are deployed in a dedicated subnet on either side. Subnet peering is established between the two firewall subnets. No virtual network peering is used in this architecture. Three different groups of RISE workload VMs with communication paths to and from the the respective VM group and RISE firewall.
:::image-end:::
