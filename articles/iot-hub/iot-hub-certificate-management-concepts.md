...
A Certificate Signing Request (CSR) is a digitally signed message generated by a client (ex: IoT Device) to request a signed certificate from a Certificate Authority (CA). Each CSR contains the device’s public key and identifying information (such as its Registration ID), and it is signed with the corresponding private key to prove key ownership.

Each CSR must meet the PKI’s policy requirements, including approved key algorithms, key sizes, and subject field formats. If a device generates a new private key, it must also generate a new CSR. When the CA verifies and approves the CSR, it issues an X.509 certificate that binds the device’s identity to its public key. This process ensures that only legitimate devices—those that can demonstrate possession of their private key—receive trusted certificates.

### Certificate signing request requirements in Device Provisioning Service

In certificate management, CSRs are to be delivered during device provisioning or re-provisioning. DPS requires that all CSRs must be submitted in Base64-encoded DER format, conforming to the PKCS #10 standard, excluding PEM headers and footers. The Common Name (CN) must match the device registration ID.
