...
In certificate management, CSRs are to be delivered during device provisioning or re-provisioning. Device Provisioning Service requires Base64-enconded DER certificate signing request in PKCS #10 format, excluding PEM headers and footers. The Common Name (CN) must match the device registrationID
