...
Automatic device management works by updating a set of device twins or module twins with desired properties and reporting a summary based on twin reported properties. It introduces a new class and JSON document called a _Configuration_ that has three parts:
...
1. Give your configuration a unique name that is up to 128 characters long. Lowercase letters and the following special characters are allowed: `-+%_*!'`. Spaces aren't allowed.
...
Since multiple configurations can target the same device or module, each configuration needs a priority number. If there's ever a conflict, the configuration with the highest priority wins. 
...
Metrics provide summary counts of the various states that a device or module can report back after applying configuration content. For example, you can create a metric for pending settings changes, a metric for errors, and a metric for successful settings changes.
...
3. Inspect the configuration list. For each configuration, you can view the following details:
   ...
   * **System metrics** - metrics that are calculated by IoT Hub and can't be customized by developers. _Targeted_ specifies the number of device twins that match the target condition. _Applies_ specifies the number of device twins that are modified by the configuration, which can include partial modifications if a separate, higher priority configuration also made changes. 
...
* If a twin currently running this configuration no longer meets the target condition, the settings from the configuration are removed and the twin are modified by the next highest priority configuration. 
...
5. A prompt asks you to confirm.
