...
Once the DDoS Ruleset is applied to a policy that is attached to an Azure Front Door Premium profile, traffic baselines are calculated based on a rolling window. For profiles that have received traffic for at least 50% of the past seven days, the ruleset immediately calculates baselines and detects spikes. If the Front Door profile didn't receive traffic for at least 50% of the past seven days, the ruleset won't detect or block attacks until sufficient traffic is present to determine baselines.
...
The HTTP DDoS Ruleset learns both a global profile threshold and a per IP address threshold. Until the profile threshold for requests is breached, the IP based thresholds won't trigger. Once the profile threshold has been breached, the IPs thresholds are enforced and any IP address that breaches the baseline will be throttled at the rate of the learned baseline. This allows the ruleset to not block spikes from a few IP addresses if they don’t cause the total request rate the profile is seeing to cross the threshold.
...
The HTTP DDoS Ruleset currently has two rules, and each can be configured with different sensitivity and action settings.  Each rule will maintain different traffic baselines for traffic that matches the rule criteria. 
...
- When each IP first breaches a threshold, a log entry is recorded for that IP Address as a Block action for the HTTP DDoS Ruleset and increment the WAF Managed Rule Match Metric. 
