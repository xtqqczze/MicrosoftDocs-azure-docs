...
>
> Azure File Sync requires a minimum SAS expiry interval of 3 days when the network access control list (ACL) is enabled. Configuring the “Upper limit for SAS expiry interval” to less than 3 days prevents the File Sync agent from renewing its SAS token,
> which will interrupt sync and cloud recall operations. Set the upper limit to 3 days or greater.
