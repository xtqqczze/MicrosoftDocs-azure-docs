...
ms.date: 12/09/2025
ms.topic: quickstart
#Customer intent: As a customer, I want to enable Azure Change Tracking and Inventory (CTI) so that I can further use the Azure CTI services.
...
This article describes how you can enable [Azure Change Tracking and Inventory](overview-monitoring-agent.md) (CTI) for single and multiple Azure virtual machines (VMs) from the Azure portal.
...
## Enable Azure CTI for single and multiple Azure Arc-enabled VMs from the Azure portal
...
#### Enable Azure CTI for a single Azure VM by using the portal

To enable Azure CTI for a single Azure VM by using the portal, follow these steps:
...
#### Enable Azure CTI for a single Azure Arc-enabled VM by using the portal

To enable Azure CTI for a single Azure Arc-enabled VM by using the portal, follow these steps:
...
### Enable Azure CTI for multiple VMs by using the Azure portal and the Azure CLI

This section provides detailed procedures on how you can enable Azure CTI on multiple Azure VMs and Azure Arc-enabled VMs.

#### Multiple Azure VMs by using the portal

To enable Azure CTI for multiple Azure VMs by using the portal, follow these steps:
...
1. Select the VMs to which you intend to enable Change Tracking, and select **Services** > **Change Tracking**.
   ...
   > [!NOTE]
   > You can enable Azure CTI on up to 250 VMs at a time.
...
A notification appears in the upper-right corner of the pane that indicates the status of the Azure CTI deployment.

#### Multiple Azure Arc-enabled VMs by using the CLI

To enable the Azure CTI on Azure Arc-enabled servers, ensure that the custom Change Tracking DCR is associated to the Azure Arc-enabled VMs.

To associate the DCR to the Azure Arc-enabled VMs, follow these steps:

1. [Create a Change Tracking DCR](create-data-collection-rule.md). You can also use an existing DCR that collects data for Azure CTI. These rules are part of Azure Monitor, which helps you manage and monitor your systems.
