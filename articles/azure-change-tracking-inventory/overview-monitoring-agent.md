...
This article provides an overview of Azure Change Tracking and Inventory (CTI) using Azure Monitoring Agent (AMA). It describes the key features, benefits, supported operating systems, and limitations of the service.
...
> [!IMPORTANT]
> - We recommend that you use Azure CTI with the Change tracking extension version 2.20.0.0 (or above).
...
1. **Change Tracking**
    ...
    > [!NOTE]
    > Azure CTI data is logged for both system-level and user-level applications. System-level data is always logged, but user-level applications appear when a user logs into a machine; if the user logs out, those applications are marked as 'Removed'.
