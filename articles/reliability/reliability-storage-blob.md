...
This article describes reliability support in [Blob Storage](/azure/storage/blobs/storage-blobs-overview). When you use Blob Storage with a hierarchical namespace, the service is sometimes called *Azure Data Lake Storage Gen2*, and this article covers that service too. The article describes intra-regional resiliency via [availability zones](#availability-zone-support) and [multi-region deployments](#multi-region-support).
