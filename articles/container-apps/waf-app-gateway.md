...
> [!NOTE]
> When configuring header rewrite rules, make sure to use the correct variable syntax. Server variables must use the appropriate prefix (such as `http_req_` for request headers). For troubleshooting rewrite rule configuration errors, see [Troubleshoot Application Gateway rewrite rules](../application-gateway/rewrite-http-headers-url.md#troubleshoot-rewrite-rules).
