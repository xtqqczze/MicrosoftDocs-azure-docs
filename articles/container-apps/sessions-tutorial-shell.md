---
title: "Tutorial: Run commands in a shell session in Azure Container Apps"
...
# Tutorial: Run shell commands in a shell session in Azure Container Apps (preview)

This tutorial demonstrates how to execute shell commands in Azure Container Apps dynamic sessions using an HTTP API.
...
> * Pass in shell commands for the container app to run
...
Use the `az containerapp sessionpool create` command to create a session pool that is responsible for executing shell commands.
...
```bash
curl --request POST --url "$SESSION_POOL_MANAGEMENT_ENDPOINT/executions?api-version=2025-02-02-preview&identifier=83c0d749-9ce8" --header "$AUTH_HEADER" --header 'content-type: application/json' --data '{
