...
Consider an orchestration that calls 3 different activities. Here's how the Durable Task Scheduler processes each action:
...
1. Orchestration Start → 1 action
...
The Dedicated SKU provides predictable performance and pricing through pre-allocated Compute Units (CUs).
...
#### Small 5 activity example

For orchestrations with 5 activities plus error handling, averaging 12 actions per orchestration (orchestrator invocations + activity invocations).

Calculation running 20 million orchestrations per month:

- Monthly actions: 20M × 12 = 240M actions- 

- Actions per second: 240M ÷ 2,628,000 ≈ 91 actions/second
...
- Monthly actions: 500M × 13 = 6.5B actions

- Actions per second: 6.5B ÷ 2,628,000 ≈ 2,854 actions/second

- Required CUs: 2,473 ÷ 2,000 = 1.23 → 2 CUs sufficient

#### SaaS platform example

A SaaS platform supporting 800 million orchestrations monthly, each with an average of 15 actions (e.g., user interactions, background processing, and external API calls).

Calculation:

- Monthly actions: 800M × 15 = 12B actions

- Actions per second: 12B ÷ 2,628,000 ≈ 4,567 actions/second

- Required CUs: 4,567 ÷ 2,000 = 2.28 → 3 CUs sufficient
...
- Monthly actions: 10K × 3 = 30K actions

- Cost: 30K × \$0.003 = \$90/month

#### Bursty workloads example

An e-commerce application experiences dynamic workload scaling during promotional sales events, especially on weekends. It uses an orchestration comprising of 7 total actions, which executes approximately 20,000 times per month.

Cost Calculation:

- Monthly actions: 20K × 7 = 140K actions

- Cost: 140K × \$0.003 = \$420/month
