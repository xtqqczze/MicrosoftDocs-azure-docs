...
titleSuffix: How to choose between Azure Key Vault, Azure Key Vault Managed HSM, and Azure Payment HSM
...
Azure offers several solutions for cryptographic key storage and management in the cloud: Azure Key Vault (standard and premium offerings), Azure Key Vault Managed HSM, and Azure Payment HSM. This article helps you choose the right solution based on your scenarios, requirements, and industry.
...
- _SSL/TLS Offload_ is supported on Azure Managed HSM. Customers have improved high availability, security, and the best price point on Azure Managed HSM for F5 and Nginx.
- _Lift and shift_ refer to scenarios where a PKCS11 application on-premises is migrated to Azure Virtual Machines and running software such as Oracle TDE in Azure Virtual Machines. Lift and shift requiring payment PIN processing is supported by Azure Payment HSM.
...
Azure provides multiple key management solutions to allow customers to choose a product based on both high-level requirements and management responsibilities. There is a spectrum of management responsibilities ranging from Azure Key Vault and Azure Managed HSM having less customer responsibility, followed by Azure Payment HSM having the most customer responsibility.
...
|  | **AKV Standard** | **AKV Premium** | **Azure Key Vault Managed HSM** | **Azure Payment HSM** |
| --- | --- | --- | --- | --- |
| What level of **compliance** do you need? | FIPS 140-2 level 1 | FIPS 140-2 level 2 | FIPS 140-2 level 3, PCI DSS, PCI 3DS | FIPS 140-2 level 3, PCI HSM v3, PCI PTS HSM v3, PCI DSS, PCI 3DS, PCI PIN |
| Do you need **key sovereignty**? | No | No | Yes | Yes |
| What kind of **tenancy** are you looking for? | Multitenant | Multitenant | Single Tenant | Single Tenant |
| What are your **use cases**? | Encryption at Rest, CMK, custom | Encryption at Rest, CMK, custom | Encryption at Rest, TLS Offload, CMK, custom | Payment PIN processes, custom |
| Do you want **HSM hardware protection**? | No | Yes | Yes | Yes |
| What is your **budget**? | $ | $$ | $$$ | $$$$ |
| Who takes responsibility for **patching and maintenance**? | Microsoft | Microsoft | Microsoft | Customer |
| Who takes responsibility for **service health and hardware failover**? | Microsoft | Microsoft | Shared | Customer |
| What kind of **objects** are you using? | Asym Keys, Secrets, Certs | Asym Keys, Secrets, Certs | Asym/Sym Keys | Local Master Key |
| **Root of trust control** | Microsoft | Microsoft | Customer | Customer |
...
| --- | --- | --- |
| I am an enterprise or an organization with strict security and compliance requirements (ex: banking, government, highly regulated industries). | Azure Key Vault Managed HSM | Azure Key Vault Managed HSM provides FIPS 140-2 Level 3 compliance, and it is a PCI compliant solution for ecommerce. It supports encryption for PCI DSS 4.0. It provides HSM backed keys and gives customers key sovereignty and single tenancy. |
...
| I am an IaaS customer wanting to move my application to use Azure VM/HSMs. | Azure Key Vault Managed HSM | Azure Key Vault Managed HSM supports IaaS scenarios and provides FIPS 140-2 Level 3 compliance with key sovereignty. |
