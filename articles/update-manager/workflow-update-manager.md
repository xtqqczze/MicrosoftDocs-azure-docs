...
When an Update Manager operation is enabled or triggered on your Azure or Arc-enabled server, Update Manager installs an [Azure extension](/azure/virtual-machines/extensions/overview) or [Azure Arc-enabled servers extensions](/azure/azure-arc/servers/manage-vm-extensions) respectively on your machine to manage the updates.
...
Customer doesn't have to explicitly install the extension and its lifecycle as it is managed by Update Manager including installation and configuration. The Update Manager extension is installed and managed by using the below agents, which are required for Update Manager to work on your machines:
...
> [!NOTE]
> Azure Arc connectivity is a prerequisite for Update Manager, non-Azure machines including Azure Arc-enabled VMWare, SCVMM, etc.
...
Update Manager honors the update source settings on the machine and will fetch updates accordingly. Update Manager doesn't publish or provide updates.

#### [Windows](#tab/update-win)

If the [Windows Update Agent (WUA)](/windows/win32/wua_sdk/updating-the-windows-update-agent) is configured to fetch updates from Windows Update repository or Microsoft Update repository or [Windows Server Update Services](/windows-server/administration/windows-server-update-services/get-started/windows-server-update-services-wsus) (WSUS), Update Manager honors these settings. For more information, see how to [configure Windows Update client](/windows-server/administration/windows-server-update-services/get-started/windows-server-update-services-wsus). By default, *it's configured to fetch updates from Windows Updates repository*.

#### [Linux](#tab/update-lin)

If the package manager points to a public YUM, APT or Zypper repository or a local repository, Update Manager honors the settings of the package manager.  
...
1. Retrieve the assessment information about status of system updates for it specified by the Windows Update client or Linux package manager.
1. Initiate the download and installation of updates with the Windows Update client or Linux package manager.
...
Update Manager uses Windows Update Agent (WUA) APIs to install updates. The updates installed using WUA APIs do not reflect in Windows Update page within the Settings app in the machine and hence the updates installed through Update Manager are not visible in Windows Update page in the Settings app. The Windows Update page in the Settings app shows the progress and history of updates managed by the Windows Update orchestrator workflow. [Learn more](/windows/win32/wua_sdk/portal-client#purpose).
...
In Update Manager, patches are installed in the following manner:
