...
* If Traffic Manager finds an endpoint match and that endpoint is in the **Stopped** state, Traffic Manager returns a NODATA response. In this case, Traffic Manager makes no further lookups higher up in the geographic region hierarchy. This behavior also applies to nested endpoint types when the child profile is in the **Stopped** or **Disabled** state.
