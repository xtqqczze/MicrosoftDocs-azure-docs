...
First create a new Application Gateway as you would usually through the portal - there are no extra steps needed in the creation to enable mutual authentication. For more information on how to create an Application Gateway in portal, check out our [portal quickstart tutorial](./quick-create-portal.md).

## Configure mutual authentication strict mode

To configure an existing Application Gateway with mutual authentication strict mode, you'll need to first go to the **SSL settings** tab in the Portal and create a new SSL profile. When you create an SSL profile, you'll see two tabs: **Client Authentication** and **SSL Policy**. The **Client Authentication** tab is where you'll upload your client certificate(s). The **SSL Policy** tab is to configure a listener specific SSL policy - for more information, check out [Configuring a listener specific SSL policy](./application-gateway-configure-listener-specific-ssl-policy.md).

> [!IMPORTANT]
> Ensure that you upload the entire client CA certificate chain in one file, and only one chain per file.

1. Search for **Application Gateway** in portal, select **Application gateways**, and select on your existing Application Gateway.
...
**Associate the SSL profile with a listener (Strict)**
...
3. Select **Add listener** if you don't already have an HTTPS listener set up. If you already have an HTTPS listener, select it from the list. 
...
8. Select **Add** to save your new listener with the SSL profile associated to it. 

    > [!div class="mx-imgBorder"]
    > ![Associate SSL profile to new listener](./media/mutual-authentication-portal/mutual-authentication-listener-portal.png)

## Configure mutual authentication passthrough  mode

To configure an existing Application Gateway with mutual authentication passthrough mode, you'll need to first go to the **SSL settings** tab in the Portal and create a new SSL profile. When you create an SSL profile, you'll see two tabs: **Client Authentication** and **SSL Policy**. You have to choose **Client Authentication** to setup passthrough mode. **SSL Policy** is optional

> [!IMPORTANT]
> For passthrough mode  you don't need  to upload CA certificate
...
4. Enter a name under **SSL Profile Name**. In this example, we call our SSL profile *applicationGatewaySSLPassthroughProfile*. 

5. Stay in the **Client Authentication** tab. 

   Select Passthrough
   > [!NOTE]
   > No CA certificate is needed in Passthrough Mode


6. Consider adding a listener specific policy (Optional). See instructions at [setting up listener specific SSL policies](./application-gateway-configure-listener-specific-ssl-policy.md).

8. Select **Add** to save.
    > [!div class="mx-imgBorder"]
    > ![Add client authentication to SSL profile](./media/mutual-authentication-portal/mutual-authentication-portal.png)

**Associate the SSL profile with a listener (Passthrough)**
...
3. Select **Add listener** if you don't already have an HTTPS listener set up. If you already have an HTTPS listener, click on it from the list. 
...
In the case that your client CA certificate expired, you can update the certificate on your gateway through the following steps: 
...
1. Repeat steps 2-4 above with any other SSL profile that was using the same expired client certificate. You'll be able to choose the new certificate you uploaded in step 3 from the dropdown menu in other SSL profiles.
