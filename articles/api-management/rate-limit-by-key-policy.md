...
* In the v2 tiers, when configuring this policy at more than one scope and using the same `counter-key` value, ensure that the `renewal-period` and `calls` values are consistent in all instances of the policy. Inconsistent values may result in unpredictable behavior due to changes in the rate-limiting implementation in v2 compared to classic tiers. [Learn more](api-management-sample-flexible-throttling.md#rate-limits)
