...
description: Step-by-step migration guide for Amazon API Gateway to Azure API Managment. Includes assessment, preparation, process, and validation.
...
If you currently use Amazon API Gateway and plan to migrate your workload to Azure, this guide can help you understand the migration process, feature mappings, and best practices. On Azure, [Azure API Management](api-management-key-concepts) provides API gateway capabilities including API request and response routing, authorization and access control, monitoring and governance, and API version management.
...
:::image type="content" source="media/migrate-amazon-api-gateway-to-api-management/image1.png" alt-text="Diagram of example Amazon API Gateway architecture.":::  
...
:::image type="content" source="media/migrate-amazon-api-gateway-to-api-management/image2.png" alt-text="Diagram of example architecture migrated to Azure API Management.":::
...
|---|---|---|
| [Private VPC endpoints](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-private-api-create.html) | Premium (classic): [Deploy Azure API Management instance to internal VNet](api-management-using-with-internal-vnet.md)<br><br>-OR-<br><br>Premium v2: [Inject API Management in virtual network - Premium v2](inject-vnet-v2) | Configure dedicated subnets for backends in a virtual network where the API Management service is injected. |
...
| [CloudWatch logs](https://docs.aws.amazon.com/apigateway/latest/developerguide/set-up-logging.html) and [CloudTrail logs](https://docs.aws.amazon.com/apigateway/latest/developerguide/cloudtrail.html) | [Azure Monitor logs](monitor-api-management) | Configure diagnostic settings to send API Management logs to a Log Analytics workspace for built-in analytics and custom analysis. Consider deploying Application Insights for added operational monitoring. |
...
- Custom domains are supported on API Management, but if using Application Gateway WAF in front, the custom domain and TLS certificate are applied at the Applicaton Gateway layer. Application Gateway must terminate TLS (no pass-through), then forward traffic to API Management. This "double termination" is a consideration in the architecture.
...
| **API Gateway scope** | **API Management equivalent** |
|----|----|
| API resource | API |
...
| [API stages](https://docs.aws.amazon.com/apigateway/latest/developerguide/how-to-deploy-api.html) | [API versions](api-management-versions) | Create API versions in API Management mapping |
...
- **Plan for infrastructure setup**
  ...
  - Understand the implications of private verusus public exposure of the target API Management system, especially around DNS and traceability.
...
- **Understand and document the source APIs under management**
  ...
  - Identify all services integrated with API Gateway (e.g., Lambdas, Application Load Balancers and Network Load Balancers, Kubernetes workloads).
...
- **Export AWS configurations where possible**
  ...
  Capture artifacts including [CloudFormation templates](api-management-versions) which might be exported to Terraform using external tools, facilitating mapping to Azure [Bicep/ARM template/Terraform templates](/azure/templates/microsoft.apimanagement/allversions).
...
> If needed, build the Azure tenant and core infrastructure (core networking, ingress, security) before migrating API Gateway and APIs. You can set up the environment using an Azure landing zone architecture suitable for your migration.
...
- **Custom Hostname**: Add your custom domain to API Management and upload the SSL certificate (or use Key Vault references). This can be done now or just before production cutover. If using Application Gateway (recommended), configure it with the custom domain and certificate, and point it to APIM’s internal endpoint.

<!-- -->

- **Establish Networking and Security**: Make sure Application Gateway (or other Azure entry point) is configured to forward requests to API Management. Set up **NSG rules** or firewall rules so that APIM can reach back-end services (such as your Azure backends or even to AWS services if initially pointing to them).

- Enable **Managed Identity** on API Management to call Azure services securely (like Key Vault for certificates or Function Apps with auth).
...
- **Start with a Small Sample Workload:** Use a test API to validate basic gateway functionality in API Management before recreating APIs from API Gateway.

<!-- -->

- **Import into APIM:** Use Azure Portal or scripts to **import OpenAPI** definitions from API Gateway as new APIs in API Management. During import, API Management automatically creates the structure of APIs and operations. Initially, set the **backend URL** for each API to point to the current backend (which for now could still be an AWS endpoint or public endpoint). For example, if in AWS the API Gateway forwarded to a Lambda, you might set the API Management backend to the equivalent API in API Gateway (you’ll later change this if you migrate the Lambda to Azure) or to an equivalent Azure Function if already migrated. If the backend was an AWS ALB or endpoint, API Management can call it over internet.

<!-- -->

- If you have a large number of APIs, you can use Azure API Center over time to catalog the APIs that are migrated to Azure API Management and the ones that remain in API Gateway.

- Consider migrating or refactoring backend services (e.g., Function Apps, AKS workloads) after validating infrastructure. See guidance in the Azure Migration Hub

<!-- -->

- **Set up Authentication**

  - **Set up subscriptions and Products**: If your APIs required **API keys** in API Gateway (x-api-key header), decide how to handle that in API Management. One approach is to make those APIs only accessible to users with a subscription to a product. **Create initial products** in API Management (you can have a one-to-one with AWS usage plans or reorganize logically).

  <!-- -->

  - **Create User Groups** in API Management to mirror how you share APIs with developers.

  - **Named Values**: Import any configuration values (like API keys for back-end services, endpoints, etc.) that were in API Gateway stage variables into APIM Named Values. For sensitive values, use Azure Key Vault integration.
...
- **Transformation and Policy Mapping**
  ...
  - Handle transformations (e.g., schema mapping, enrichment) case-by-case.
...
- **Iterative testing and API migration**

With the APIs configured in API Management, thorough testing is critical. This phase is expected to be iterative.

- **Functional Testing:** For each API:

<!-- -->

- Call the new Azure API Management endpoint (via the Azure portal’s test console or client tools) and compare responses to the Amazon API Gateway endpoint. Check for expected status codes, headers, and body.

- If differences are found, adjust APIM policies or configuration accordingly.
...
- **Web Application Firewall (WAF)**: API Management will not automatically block bad payloads that AWS WAF would have. If you set up Application Gateway WAF, make sure API Management is only accessible through the WAF and that WAF rules replicate AWS WAF restricutions.

- **Event Streams**: If any CloudWatch alarms or events were tied to API Gateway (for example, on certain error patterns), set up equivalent alerts in Azure Monitor for APIM (for example, an alert on APIM 5XX rate).

- **Automation**: If you have CI/CD pipelines, integrate API Management into them. For example, you might store your API Management configurations (APIs and policies) in source control using *Infrastructure as Code* approaches (like ARM/Bicep templates or an APIOps methodology). This ensures that future changes to the APIs can be deployed consistently and version-controlled.
...
- **Production rollout**
  ...
  - DNS switch**:** The most common approach is to switch the DNS entry of your custom API Gateway domain to point to the new Azure endpoint. For example, if your domain api.example.com was mapped to Amazon API Gateway, you would update its CNAME or A record to point to the Azure API Management hostname or to the front-end (like Application Gateway) domain.  
    ...
    TTL considerations**:** Lower the DNS TTL beforehand so that clients pick up changes quickly. When ready, change the DNS. It can take minutes to hours to propagate, during which time some traffic might still go to AWS and some to Azure. If an immediate cut is needed, you can use an alternate cutover method such as a reverse-proxy approach.

  - **Alternate Cutover Methods:** Sometimes, instead of DNS, organizations use a **reverse proxy/gateway flip** – for instance, keep the public DNS the same but initially have Application Gateway forward requests to *Amazon API Gateway* (via its URL), and when flipping, point it to API Management internally. This is more complex but can achieve an instantaneous switch. Another method if using Azure Front Door or Traffic Manager is to reweight traffic from one backend (AWS) to another (Azure) gradually.

  - **Monitoring During Cutover:** As soon as the switch happens, closely monitor requests to both the Azure API Management instance the Amazon API Gateway. Monitor API Management metrics (requests, latency, CPU, capacity memory) in real-time via Azure Portal or whatever dashboard you set up. Also use Azure Monitor to watch for any spikes in errors such as 4XX/5XX responses.
...
The migration can be considered successful when the migrated system meets validation criteria and when all production traffic is served by Azure API Management with no significant regression in functionality or performance
...
- Validate operational metrics
  ...
  - Monitoring – Confirm that Azure Monitor and Application Insights (if configured) dashboards capture data for all API calls
...
- Layer in additional capabilities such as external caching and monitoring capabilities beyond Azure Monitor such as Datadog, or policies in API Managment that aren’t available in API Gateway

- Decommission API Gateway after a period when it receives zero traffic and the API Management instance has met the validation criteria. Typically, you will run both in parallel for a time (with Azure taking all traffic) just for safety. It’s good practice to wait through at least one full business cycle or peak traffic period to ensure the new system handles it.
