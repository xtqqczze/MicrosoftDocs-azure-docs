...
> [!NOTE]
> The configuration steps in this article show how to semantic caching for APIs added to API Management from Azure OpenAI in Azure AI Foundry models. These steps can be generalized to enable semantic caching for corresponding large language model (LLM) APIs available through the [Azure AI Model Inference API](/rest/api/aifoundry/modelinference/) or with OpenAI-compatible models served through third-party inference providers. 

## Prerequisites

* One or more Azure OpenAI in Azure AI Foundry model deployments must be added as APIs to your API Management instance. For more information, see [Add an Azure OpenAI API to Azure API Management](azure-openai-api-from-specification.md).
