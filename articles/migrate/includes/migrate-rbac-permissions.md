...
To register an Azure Migrate appliance or an ASR replication appliance users must have additional [Application Developer role](../active-directory/roles/permissions-reference?view=migrate#application-developer) at Microsoft Entra ID level. For detailed guidance on these role assignments, refer [Prepare Azure accounts using built-in roles](/azure/migrate/prepare-azure-accounts/prepare-azure-accounts).
