...
1. On the **Backup policy** tab, under **Backup policy**, select the policy you want to use for data retention, and then select **Next**.
   If you want to create a new backup policy, select **Create new**. learn how to [create a backup policy](/azure/backup/azure-data-lake-storage-backup-create-policy-quickstart?pivots=client-portal).
